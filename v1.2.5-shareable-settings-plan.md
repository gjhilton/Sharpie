...
- Badges are always visible (even when Options section is collapsed)
...
- Success feedback is shown after copying
...
- [ ] Add optional `summaryComponent` prop to component signature
...
```
LandingScreen
├── DisclosureSection (title="Options", summaryComponent={<OptionsSummary />})
│   ├── [Title + Expand Button]
│   ├── OptionsSummary ← NEW
│   │   ├── Badge: Game Mode
...
- Share history (recently shared configurations)
- Preset configurations ("Easy mode", "Expert mode", etc.)
